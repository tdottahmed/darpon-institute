...
export default function Show({ videoBlog, relatedVideoBlogs }) {
    ...
    const youtubeId = videoBlog.video_type === 'youtube' ? getYoutubeId(videoBlog.video_url) : null;
                    ...
                    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        ...
                        <nav className="flex mb-8" aria-label="Breadcrumb">
                            <ol className="flex items-center space-x-2">
                                ...
                                <li>
                                    <Link href={route("video_blogs.index")} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                                        Video Blogs
                                    </Link>
                                </li>
                                <li className="text-gray-400">/</li>
                                <li className="text-gray-900 font-medium dark:text-white truncate max-w-xs">{videoBlog.title}</li>
                            </ol>
                        </nav>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div className="lg:col-span-2">
                                ...
                                <div className="aspect-video w-full overflow-hidden rounded-2xl bg-black shadow-lg mb-8 relative z-10">
                                    {videoBlog.video_type === 'youtube' && youtubeId ? (
                                        <iframe
                                            ...
                                            allowFullScreen
                                        ></iframe>
                                    ) : videoBlog.video_type === 'upload' && videoBlog.video_file ? (
                                        <video 
                                            controls 
                                            className="h-full w-full"
                                            poster={videoBlog.thumbnail ? `/storage/${videoBlog.thumbnail}` : null}
                                        >
                                            <source src={`/storage/${videoBlog.video_file}`} type="video/mp4" />
                                ...
                                <div className="space-y-4">
                                     <div className="flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
                                        <span>{new Date(videoBlog.created_at).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                                        {videoBlog.tags && <span className="text-gray-300">•</span>}
                                        <div className="flex gap-2">
                                            {videoBlog.tags && videoBlog.tags.map((tag, i) => (
                                                <span key={i} className="font-medium text-primary-600 dark:text-primary-400 uppercase tracking-wider">{tag}</span>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div className="prose prose-lg text-gray-600 dark:text-gray-300 dark:prose-invert max-w-none mt-6">
                                        {videoBlog.long_description ? parse(videoBlog.long_description) : videoBlog.short_description}
                                    </div>
                                </div>
                            </div>
