      ...
      <form action="{{ route('admin.books.update', $book) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="space-y-4 rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50">
          ...
          <x-forms.textarea name="focus_keyphrase_options" label="Focus keyphrase" rows="2" :value="old(
              ...
              is_array($book->focus_keyphrase_options) && count($book->focus_keyphrase_options) ? implode(', ', $book->focus_keyphrase_options) : '',
          )" :error="$errors->first('focus_keyphrase_options')"
                            help="Optional. One phrase or several—separate with commas or new lines." />
          <x-forms.input name="seo_title" label="SEO title" :value="old('seo_title', $book->seo_title)" :error="$errors->first('seo_title')"
                         help="Shown in search results; leave blank to use the book title. Often 50–60 characters."
                         maxlength="255" />
          <x-forms.textarea name="meta_description" label="Meta description" :value="old('meta_description', $book->meta_description)" rows="3"
