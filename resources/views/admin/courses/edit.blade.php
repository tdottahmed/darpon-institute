      ...
      <form action="{{ route('admin.courses.update', $course) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="space-y-4 rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50">
          ...
          <x-forms.textarea name="focus_keyphrase_options" label="Focus keyphrase" rows="2" :value="old(
              ...
              is_array($course->focus_keyphrase_options) && count($course->focus_keyphrase_options) ? implode(', ', $course->focus_keyphrase_options) : '',
          )" :error="$errors->first('focus_keyphrase_options')"
                            help="Optional. One phrase or several—separate with commas or new lines." />
          <x-forms.input name="seo_title" label="SEO title" :value="old('seo_title', $course->seo_title)" :error="$errors->first('seo_title')"
                         help="Shown in search results; leave blank to use the course title. Often 50–60 characters."
                         maxlength="255" />
          <x-forms.textarea name="meta_description" label="Meta description" :value="old('meta_description', $course->meta_description)" rows="3"
