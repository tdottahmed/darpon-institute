...
@section('content')
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900">Create Payment Gateway</h1>
        <p class="mt-1 text-sm text-gray-600">Add a new payment method</p>
      </div>
    ...
    <x-card variant="elevated">
      <form action="{{ route('admin.payment-gateways.store') }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <x-forms.input name="name" label="Gateway Name" :value="old('name')" required :error="$errors->first('name')"
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <x-forms.input name="account_number" label="Account/Phone Number" :value="old('account_number')" :error="$errors->first('account_number')"
                         placeholder="e.g. 017XXXXXXXX" />
          <x-forms.input name="account_name" label="Account Holder Name" :value="old('account_name')" :error="$errors->first('account_name')"
        ...
        <div>
          ...
          <textarea name="instructions" id="instructions" rows="4"
                    class="block w-full rounded-lg border-gray-200 bg-white shadow-sm focus:border-primary-500 focus:ring-primary-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300"
                    placeholder="Enter payment instructions for users...">{{ old('instructions') }}</textarea>
          @error('instructions')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <x-forms.input name="order" label="Display Order" type="number" min="0" :value="old('order', 0)"
