  ...
  <nav class="-mb-px flex space-x-8 overflow-x-auto" aria-label="Tabs">
    @foreach ($tabs as $tabKey => $tabLabel)
      <a href="{{ $baseUrl }}?tab={{ $tabKey }}"
         class="{{ $activeTab === $tabKey ? 'border-primary-500 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap border-b-2 px-1 py-4 text-sm font-medium transition-colors">
        {{ $tabLabel }}
      </a>
    @endforeach
  </nav>
