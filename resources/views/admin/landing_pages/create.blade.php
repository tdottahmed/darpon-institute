  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-600">Create a custom landing page for a course or book</p>
      </div>
    ...
    <form action="{{ route('admin.landing-pages.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <div>
            <x-forms.select name="product_type" label="Product Type" :options="[
                ...
                'course' => 'Course',
                'book' => 'Book',
            ]" :value="old('product_type')" required :error="$errors->first('product_type')" id="product-type" />
          </div>
          <div>
            <x-forms.select name="product_id" label="Select Product" :options="[]" :value="old('product_id')" required
      ...
      <x-card variant="elevated">
        ...
        <div class="space-y-6">
          <x-forms.input name="hero_title" label="Hero Title" :value="old('hero_title')" :error="$errors->first('hero_title')"
                         placeholder="Main headline for the hero section" />
          <x-forms.textarea name="hero_subtitle" label="Hero Subtitle" :value="old('hero_subtitle')" rows="3"
                            :error="$errors->first('hero_subtitle')" placeholder="Supporting text below the title" />
          <x-forms.image-uploader name="hero_image" label="Hero Image" :value="old('hero_image')" accept="image/*"
                                  maxSize="2MB" :error="$errors->first('hero_image')" />

          <div x-data="{ videoType: '{{ old('hero_video_type', '') }}' }">
            ...
            <div x-show="videoType === 'url'" class="space-y-2">
              <x-forms.input name="hero_video" label="Video URL" :value="old('hero_video')"
      ...
      <x-card variant="elevated">
        ...
        <div class="space-y-6">
          <x-forms.rich-text name="custom_description" label="Custom Description" :value="old('custom_description')"
          ...
          <div x-data="{ videoCount: {{ count(old('custom_videos', [])) > 0 ? count(old('custom_videos', [])) : 1 }}, videos: {{ json_encode(old('custom_videos', [])) }} }">
      ...
      <x-card variant="elevated">
        ...
        <div class="space-y-6">
          <x-forms.textarea name="cta_text" label="CTA Text" :value="old('cta_text')" rows="3"
                            :error="$errors->first('cta_text')"
                            placeholder="Compelling text to encourage action (e.g., 'Limited Time Offer!')" />
          <x-forms.input name="cta_button_text" label="Button Text" :value="old('cta_button_text', 'Enroll Now')"
