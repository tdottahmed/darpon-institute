...
<div class="space-y-1" x-data="{
    tags: @js(is_array($value) ? array_values($value) : (is_string($value) && $value !== '' ? explode(',', $value) : [])),
    inputValue: '',
    syncHidden() {
        const el = this.$refs.tagHiddenField;
        if (el) {
            el.value = this.tags.join(',');
        }
    },
    init() {
        const form = this.$el.closest('form');
        if (form) {
            form.addEventListener('submit', () => this.syncHidden(), { capture: true });
        }
        this.$watch('tags', () => this.$nextTick(() => this.syncHidden()));
  ...
  <div class="mt-1">
    <input type="hidden" name="{{ $name }}" id="{{ $name }}" x-ref="tagHiddenField" value="">
